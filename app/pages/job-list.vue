<template>
  <div class="bg-white w-full mt-[100px]">
        ...
        <main class="flex-1">
          <div class="flex justify-between items-center mb-8">
            <h1 class="text-3xl font-bold">
              Recommended jobs 
              <span class="bg-white border text-sm px-3 py-1 rounded-full ml-2">{{ filteredJobs.length }}</span>
            </h1>
...
const selections = ref({ location: '', jobType: '', salary: '' })
...
const dropdownConfigs = [
  { key: 'location', label: 'All Countries', icon: '<path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />', options: ['United States', 'United Kingdom', 'Canada', 'Remote'] },
  { key: 'jobType', label: 'Job Type', icon: '<path d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />', options: ['Full-time', 'Part-time', 'Contract', 'Freelance'] },
  { key: 'salary', label: 'Salary Range', icon: '<path d="M3 10h18M7 15h1m4 0h1m-7 4h12a2 2 0 002-2V5a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />', options: ['$0 - $1k', '$1k - $5k', '$10k+'] }
]

// --- Dropdown handlers ---
const toggleDropdown = key => openDropdown.value = openDropdown.value === key ? null : key
const selectOption = (key, value) => { selections.value[key] = value; openDropdown.value = null }
...
const fetchJobs = async () => {
  try {
    ...
    jobs.value = data.map(item => {
      ...
      const companyName = attrs.company?.name || attrs.company?.data?.attributes?.name || 'No Company'

      return {
        id: item.id,
        title: attrs.title || 'No Title',
        company: companyName, // Now sending just the string "Nasa"
